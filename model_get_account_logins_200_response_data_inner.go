/*
Linode API

[Read the API documentation](https://techdocs.akamai.com/linode-api/reference/api).

API version: 4.189.2
Contact: support@linode.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"time"
)

// checks if the GetAccountLogins200ResponseDataInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetAccountLogins200ResponseDataInner{}

// GetAccountLogins200ResponseDataInner An object representing a previous successful login for a User.
type GetAccountLogins200ResponseDataInner struct {
	// When the login was initiated.
	Datetime *time.Time `json:"datetime,omitempty"`
	// The unique ID of this login object.
	Id *int32 `json:"id,omitempty"`
	// The remote IP address that requested the login.
	Ip *string `json:"ip,omitempty"`
	// True if the User that attempted the login was a restricted User, false otherwise.
	Restricted *bool `json:"restricted,omitempty"`
	// Whether the login attempt succeeded or failed.
	Status *string `json:"status,omitempty"`
	// The username of the User that attempted the login.
	Username *string `json:"username,omitempty"`
}

// NewGetAccountLogins200ResponseDataInner instantiates a new GetAccountLogins200ResponseDataInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetAccountLogins200ResponseDataInner() *GetAccountLogins200ResponseDataInner {
	this := GetAccountLogins200ResponseDataInner{}
	return &this
}

// NewGetAccountLogins200ResponseDataInnerWithDefaults instantiates a new GetAccountLogins200ResponseDataInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetAccountLogins200ResponseDataInnerWithDefaults() *GetAccountLogins200ResponseDataInner {
	this := GetAccountLogins200ResponseDataInner{}
	return &this
}

// GetDatetime returns the Datetime field value if set, zero value otherwise.
func (o *GetAccountLogins200ResponseDataInner) GetDatetime() time.Time {
	if o == nil || IsNil(o.Datetime) {
		var ret time.Time
		return ret
	}
	return *o.Datetime
}

// GetDatetimeOk returns a tuple with the Datetime field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAccountLogins200ResponseDataInner) GetDatetimeOk() (*time.Time, bool) {
	if o == nil || IsNil(o.Datetime) {
		return nil, false
	}
	return o.Datetime, true
}

// HasDatetime returns a boolean if a field has been set.
func (o *GetAccountLogins200ResponseDataInner) HasDatetime() bool {
	if o != nil && !IsNil(o.Datetime) {
		return true
	}

	return false
}

// SetDatetime gets a reference to the given time.Time and assigns it to the Datetime field.
func (o *GetAccountLogins200ResponseDataInner) SetDatetime(v time.Time) {
	o.Datetime = &v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *GetAccountLogins200ResponseDataInner) GetId() int32 {
	if o == nil || IsNil(o.Id) {
		var ret int32
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAccountLogins200ResponseDataInner) GetIdOk() (*int32, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *GetAccountLogins200ResponseDataInner) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given int32 and assigns it to the Id field.
func (o *GetAccountLogins200ResponseDataInner) SetId(v int32) {
	o.Id = &v
}

// GetIp returns the Ip field value if set, zero value otherwise.
func (o *GetAccountLogins200ResponseDataInner) GetIp() string {
	if o == nil || IsNil(o.Ip) {
		var ret string
		return ret
	}
	return *o.Ip
}

// GetIpOk returns a tuple with the Ip field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAccountLogins200ResponseDataInner) GetIpOk() (*string, bool) {
	if o == nil || IsNil(o.Ip) {
		return nil, false
	}
	return o.Ip, true
}

// HasIp returns a boolean if a field has been set.
func (o *GetAccountLogins200ResponseDataInner) HasIp() bool {
	if o != nil && !IsNil(o.Ip) {
		return true
	}

	return false
}

// SetIp gets a reference to the given string and assigns it to the Ip field.
func (o *GetAccountLogins200ResponseDataInner) SetIp(v string) {
	o.Ip = &v
}

// GetRestricted returns the Restricted field value if set, zero value otherwise.
func (o *GetAccountLogins200ResponseDataInner) GetRestricted() bool {
	if o == nil || IsNil(o.Restricted) {
		var ret bool
		return ret
	}
	return *o.Restricted
}

// GetRestrictedOk returns a tuple with the Restricted field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAccountLogins200ResponseDataInner) GetRestrictedOk() (*bool, bool) {
	if o == nil || IsNil(o.Restricted) {
		return nil, false
	}
	return o.Restricted, true
}

// HasRestricted returns a boolean if a field has been set.
func (o *GetAccountLogins200ResponseDataInner) HasRestricted() bool {
	if o != nil && !IsNil(o.Restricted) {
		return true
	}

	return false
}

// SetRestricted gets a reference to the given bool and assigns it to the Restricted field.
func (o *GetAccountLogins200ResponseDataInner) SetRestricted(v bool) {
	o.Restricted = &v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *GetAccountLogins200ResponseDataInner) GetStatus() string {
	if o == nil || IsNil(o.Status) {
		var ret string
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAccountLogins200ResponseDataInner) GetStatusOk() (*string, bool) {
	if o == nil || IsNil(o.Status) {
		return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *GetAccountLogins200ResponseDataInner) HasStatus() bool {
	if o != nil && !IsNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given string and assigns it to the Status field.
func (o *GetAccountLogins200ResponseDataInner) SetStatus(v string) {
	o.Status = &v
}

// GetUsername returns the Username field value if set, zero value otherwise.
func (o *GetAccountLogins200ResponseDataInner) GetUsername() string {
	if o == nil || IsNil(o.Username) {
		var ret string
		return ret
	}
	return *o.Username
}

// GetUsernameOk returns a tuple with the Username field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAccountLogins200ResponseDataInner) GetUsernameOk() (*string, bool) {
	if o == nil || IsNil(o.Username) {
		return nil, false
	}
	return o.Username, true
}

// HasUsername returns a boolean if a field has been set.
func (o *GetAccountLogins200ResponseDataInner) HasUsername() bool {
	if o != nil && !IsNil(o.Username) {
		return true
	}

	return false
}

// SetUsername gets a reference to the given string and assigns it to the Username field.
func (o *GetAccountLogins200ResponseDataInner) SetUsername(v string) {
	o.Username = &v
}

func (o GetAccountLogins200ResponseDataInner) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetAccountLogins200ResponseDataInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Datetime) {
		toSerialize["datetime"] = o.Datetime
	}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Ip) {
		toSerialize["ip"] = o.Ip
	}
	if !IsNil(o.Restricted) {
		toSerialize["restricted"] = o.Restricted
	}
	if !IsNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	if !IsNil(o.Username) {
		toSerialize["username"] = o.Username
	}
	return toSerialize, nil
}

type NullableGetAccountLogins200ResponseDataInner struct {
	value *GetAccountLogins200ResponseDataInner
	isSet bool
}

func (v NullableGetAccountLogins200ResponseDataInner) Get() *GetAccountLogins200ResponseDataInner {
	return v.value
}

func (v *NullableGetAccountLogins200ResponseDataInner) Set(val *GetAccountLogins200ResponseDataInner) {
	v.value = val
	v.isSet = true
}

func (v NullableGetAccountLogins200ResponseDataInner) IsSet() bool {
	return v.isSet
}

func (v *NullableGetAccountLogins200ResponseDataInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetAccountLogins200ResponseDataInner(val *GetAccountLogins200ResponseDataInner) *NullableGetAccountLogins200ResponseDataInner {
	return &NullableGetAccountLogins200ResponseDataInner{value: val, isSet: true}
}

func (v NullableGetAccountLogins200ResponseDataInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetAccountLogins200ResponseDataInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


